...
echo "  • Background jobs: ${GREEN}Cron (every 5 minutes)${NC}"
echo "  • Cron user: ${GREEN}www-data${NC}"
echo "  • Maintenance tasks: ${GREEN}Completed${NC}"
echo ""
echo -e "${YELLOW}💡 Useful maintenance commands:${NC}"
echo "   Check cron status:  ${BLUE}sudo -u www-data php $OCC_CMD status${NC}"
echo "   Manual cron run:    ${BLUE}sudo -u www-data php $NEXTCLOUD_DIR/cron.php${NC}"
echo "   File scan:          ${BLUE}sudo -u www-data php $OCC_CMD files:scan --all${NC}"
echo "   Database optimize:  ${BLUE}sudo -u www-data php $OCC_CMD db:add-missing-indices${NC}"
echo "   View cron log:      ${BLUE}tail -f $NEXTCLOUD_DIR/data/nextcloud.log${NC}"
echo ""
